...
using Body4uHUB.Services.Application.Commands.ServiceOfferings.Update;
using Body4uHUB.Services.Application.Commands.TrainerProfiles.Update;
using Body4uHUB.Services.Application.DTOs;
using Body4uHUB.Services.Application.Queries.ServiceOfferings.GetServiceOfferingsByTrainer;
...
namespace Body4uHUB.Services.Api.Controllers
{
    ...
    public class TrainerProfilesController : ApiController
    {
        ...
        /// </summary>
        [HttpPost("{trainerId}/services/{serviceId}/reviews")]
        [Authorize]
        [ProducesResponseType(StatusCodes.Status204NoContent)]
        [ProducesResponseType(StatusCodes.Status400BadRequest)]
        [ProducesResponseType(StatusCodes.Status401Unauthorized)]
        ...
        [ProducesResponseType(StatusCodes.Status422UnprocessableEntity)]
        public async Task<IActionResult> AddReview(Guid trainerId, int serviceId, [FromBody] AddReviewRequest request)
        {
            var authContext = AuthorizationContext.Create(User.GetUserId(), User.IsAdmin());
            var command = new AddReviewCommand(trainerId, serviceId, request.OrderId, request.Rating, request.Comment, authContext);
            var result = await Mediator.Send(command);
            return HandleResult(result);
        }
